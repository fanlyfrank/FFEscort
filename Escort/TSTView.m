...
@property (assign, nonatomic) CGFloat tabIndicatorToTitle;
...
@property (assign, nonatomic) UIEdgeInsets decorationViewInset;

@property (strong, nonatomic) UIColor *tabTitleHighlightColor;
@property (strong, nonatomic) UIColor *indicatorHighlightColor;
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        //init appearance
        _tabSpace = 25.f;
        _tabIndicatorToTitle = 0.f;
        _leading = 5.f;
        _trailing = 5.f;
        _tabShadowHeight = 3.f;
        _tabHeight = 30.f;
        _tabSeparatorHeight = .5f;
        _decorationViewInset = UIEdgeInsetsMake(4, 0, 0, 0);
        
        _tabTitleHighlightColor = [UIColor blackColor];
        _indicatorHighlightColor = [UIColor blackColor];
...
- (id)dequeueReusableContentView {
    ...
    if (/*self.reuseableContentViews.count >= 3 && */self.currentSelectedIndex < self.reuseableContentViews.count - 1) {
...
- (UIView *)viewForTabIndex:(NSInteger)index {
    ...
    return self.reuseableContentViews[index];
}

- (void)selectContentAtIndex:(NSInteger)index {
    UIButton *selectedBtn = _tabButtons[index];
    [self selectTab:selectedBtn];
}
...
- (void)buildTabViews {
    ...
    if (self.dataSource) {
        ...
        if ([self.delegate respondsToSelector:@selector(tabTitleHighlightColorForTSTView:)]) {
            self.tabTitleHighlightColor = [self.delegate tabTitleHighlightColorForTSTView:self];
        }
        
        if ([self.delegate respondsToSelector:@selector(indicatorHighlightColorForTSTView:)]) {
            self.indicatorHighlightColor = [self.delegate indicatorHighlightColorForTSTView:self];
        }
...
- (void)buildDecorationViews {
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(tstview:decorationViewForSelectedTabIndex:)]) {
        for (UIButton *btn in self.tabButtons) {
            UIView *decorationView = [self.dataSource tstview:self decorationViewForSelectedTabIndex:btn.tag];
            decorationView.translatesAutoresizingMaskIntoConstraints = NO;
            
            if (self.delegate && [self.delegate respondsToSelector:@selector(edgeForDecorationViewInTSTView:)]) {
                _decorationViewInset = [self.delegate edgeForDecorationViewInTSTView:self];
            }
            
            if (decorationView) {
                [self.topTabView addSubview:decorationView];
                
                [NSLayoutConstraint constraintWithItem:decorationView
                                             attribute:NSLayoutAttributeTop
                                             relatedBy:NSLayoutRelationEqual
                                                toItem:self.topTabView
                                             attribute:NSLayoutAttributeTop
                                            multiplier:1
                                              constant:_decorationViewInset.top
                ...
                [NSLayoutConstraint constraintWithItem:decorationView
                                            ...
                                            multiplier:1
                                              constant:_decorationViewInset.left
                                                toView:self.topTabView];
                ...
                [NSLayoutConstraint constraintWithItem:decorationView
                                             ...
                                             attribute:NSLayoutAttributeHeight
                                            multiplier:0
                                              constant:decorationView.frame.size.height
                                                toView:decorationView];
                ...
                [NSLayoutConstraint constraintWithItem:decorationView
                                             ...
                                             attribute:NSLayoutAttributeWidth
                                            multiplier:0
                                              constant:decorationView.frame.size.width
                                                toView:decorationView];
...
- (void)buildContentViews {
    ...
    if (self.dataSource && self.tabsCount > 0) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(tstview:didContent:AppearAtIndex:)]) {
            [self.delegate tstview:self didContent:contentAtIndex0 AppearAtIndex:0];
        }
    }
    ...
    if (self.dataSource && self.tabsCount > 1) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(tstview:didContent:AppearAtIndex:)]) {
            [self.delegate tstview:self didContent:contentAtIndex1 AppearAtIndex:1];
        }
    }
    
    if (self.dataSource && self.tabsCount > 2) {
        UIView *contentAtIndex2 = [self.dataSource tstview:self viewForSelectedTabIndex:2];
        contentAtIndex2.frame = CGRectMake(self.frame.size.width * 2, 0, self.contentView.frame.size.width, self.contentView.frame.size.height);
        
        [self.contentView addSubview:contentAtIndex2];
        [self.reuseableContentViews addObject:contentAtIndex2];
        if (self.delegate && [self.delegate respondsToSelector:@selector(tstview:didContent:AppearAtIndex:)]) {
            [self.delegate tstview:self didContent:contentAtIndex2 AppearAtIndex:2];
        }
    }
}
...
- (void)buildTabShadowView {
    ...
    self.tabShadowView.backgroundColor = self.indicatorHighlightColor;
...
- (void)loadContentViewByIndex:(NSInteger)index {
    ...
    if (self.dataSource) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(tstview:didContent:AppearAtIndex:)]) {
            [self.delegate tstview:self didContent:contentView AppearAtIndex:index];
        }
    }
}
...
- (void)autoScrollTopTabBySwipDirctionToPage:(NSInteger)page {
    ...
    for (UIButton *btn in self.tabButtons) {
        ...
        if (btn.tag == self.currentSelectedBtn.tag) {
            ...
            [btn setTitleColor:self.tabTitleHighlightColor forState:UIControlStateNormal];
        } else {
...
- (void)addConstraintToTabButtons
{
    ...
    if ([self.delegate respondsToSelector:@selector(spaceForTopTabInTSTView:)]) {
        self.tabSpace = [self.delegate spaceForTopTabInTSTView:self];
    }
...
- (void)addConstraintToTabShadowWihtAnchor:(UIView *)anchor {
    ...
    if ([self.delegate respondsToSelector:@selector(spaceForIndicatorToTitleInTSTView:)]) {
        self.tabIndicatorToTitle = [self.delegate spaceForIndicatorToTitleInTSTView:self];
    }
    ...
    [NSLayoutConstraint constraintWithItem:self.tabShadowView
                                 ...
                                 attribute:NSLayoutAttributeBottom
                                multiplier:1
                                  constant:self.tabIndicatorToTitle
                                    toView:self.topTabView];
